...
namespace Assets.Scripts.Weapon
{
    ...
    public class WeaponController : MonoBehaviour
    {
        ...
        [Header("Magazine And Ammo")] [SerializeField] private int _ammoAmount = 200; //всего патронов
        ...
        [Header("Shoot & Reaload Speed")] [SerializeField] private const float ShootSpeed = 0.17f; //скорострельность (время на выстрел)

        [SerializeField] private const float ReloadSpeed = 1f; //скорость перезарядки (Время на перезарядку)

        [Header("Audio")] [SerializeField] private AudioClip _fireAudioClip;

        [SerializeField] private AudioClip _reloadAudioClip;

        private float _reloadTimer = 0.0f; //(не трогать)
        private float _shootTimer = 0.0f; //(не трогать)
        ...
        {
            if (Input.GetMouseButton(0) && _ammoInMag > 0 && _reloadTimer <= 0 && _shootTimer <= 0)
            {
        ...
        private void ReloadLogic()
        {
            if (Input.GetKeyDown(KeyCode.R) && _ammoInMag < MagSize)
            {
                ...
                AmmoAmount += _ammoInMag;
                _ammoInMag = _ammoAmount / MagSize > 1 ? MagSize : _ammoAmount;
